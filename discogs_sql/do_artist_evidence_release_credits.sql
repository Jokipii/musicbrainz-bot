...
WITH credits AS (
	SELECT t1.artist_gid AS gid, url(releases_extraartists.artist_name), 'Artist credited ('||t1.phrase
		||') in http://musicbrainz.org/release/'||t1.release_gid
		||' and ('||roles.credit||') in linked Discogs release '||mb_release_link.url AS note
	FROM dblink(:dblink, '
		SELECT artist.gid AS artist_gid, release.gid AS release_gid, link.link_type, lower(artist_name.name) AS artist_name, get_link_phrase(link.id) AS phrase
	...
	') AS t1(artist_gid uuid, release_gid uuid, link_type integer, artist_name text, phrase text)
